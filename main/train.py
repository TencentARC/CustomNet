...
if __name__ == "__main__":
    ...
    try:
                ...
                if in_shape[1] == 8:
                    ...
                    for input_key in input_keys:
                        ...
                        if input_weight.size() != old_state[input_key].size():
                            print(f"Manual init: {input_key}")
                            input_weight.zero_()
                            input_weight[:, :, :, :].copy_(old_state[input_key][:,:4,:,:])
                            old_state[input_key] = torch.nn.parameter.Parameter(input_weight)
                else:
                    raise ValueError("load pretrained model error")
        ...
        bs, base_lr = config.train_dataloader.batch_size, config.model.base_learning_rate
        if not cpu:
